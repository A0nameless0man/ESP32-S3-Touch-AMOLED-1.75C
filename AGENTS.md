...
- **TE 翻转引擎(最终架构,2026-08-21 收官)**:①TE 语义实测:上升沿领先行0扫描≥0.5ms(帧前导信号),TE 周期 16.95ms;②整帧/任意矩形翻转:TE2+500µs 启动,写入前沿恒领先扫描第二遍且第三遍追不上——任意 (x,y,w,h) 零撕裂;③走廊时空图判据:τ≥500+y/27.9ms 且 τ+ch·1000/W≤500+P+(y+ch)/27.9ms(W=19.7·w/466 行/ms),w=50 细高条走廊仅 38 行需切片;④脏区变化必须刷旧新并集(残留边缘是轻微撕裂根因);⑤不可行回退整帧翻转,任意绘制≤2 帧上屏;⑥CASET 手册要求 x 偶对齐(官方文档证实奇数窗口卡行坑);⑦VRR 不存在:振荡器 22MHz 固定、VFR=60Hz、无帧率命令,Idle(39h)只降色深。
- **03 工程 TE_SYNC patch(2026-08-21)**:三处改动——vendored BSP(components/esp32_s3_touch_amoled_1_75c)+ 头文件加 `BSP_LCD_TE=GPIO13` + disp_cfg 加 `.te_sync{gpio_num=13, bus_freq_hz=40MHz, data_lines=4, bpp=16}`;main.cpp 改 `TEAR_AVOID_MODE_TE_SYNC`。**关键坑**:panel_io 必须设 `io_config.flags.psram_dma_direct=true`(CO5300_PANEL_IO_QSPI_CONFIG 宏不设,否则每次 flush 从内部 DMA 堆 bounce 整块传输区→ESP_ERR_NO_MEM,co5300 2.1.0 下必现)。adapter 自动识别 TE 下降沿、periods=2(全帧 21.7ms>13ms 窗口,分两个 TE 周期)。本地构建链:~/esp/esp-idf v5.5.5(Windows 用 ~/esp/build03.bat、flash03.bat,需清 MSYSTEM);IDF 组件直链下载:`https://components-file.espressif.com/components/<ns>/<name>/<ver>/<ns>__<name>-v<ver>.zip`。
- **工厂镜像与分区表(镜像 @0x8000 解析)**:nvsfactory@0x9000 200K | nvs@0x3b000 840K | otadata@0x10d000 | phy_init@0x10f000 | **factory@0x110000 9216K** | **ota_0@0xa10000 4032K** | ota_1@0xe00000 4032K | assets@0x11f0000 9216K | storage@0x1af0000 5120K。**烧录纪律:设备运行工厂分区表,绝不 `idf.py flash`(会重写分区表砸掉 factory 原件)**;只 build + esptool `write_flash 0xa10000 app.bin`,bootloader/分区表一概不动。恢复 xiaozhi:从工厂镜像提取 ota_0 段烧回 0xa10000。
- **brookesia 五层金字塔**:①硬件层=ESP-IDF 驱动+FreeRTOS → ②移植胶水 esp_lvgl_adapter(BSP disp_cfg/te_sync)→ ③LVGL 9.5 → ④brookesia phone=OS 外壳(状态栏/导航栏/app 管理)→ ⑤App 注册表(`ESP_UTILS_REGISTER_PLUGIN_WITH_CONSTRUCTOR(BaseType, PluginType, name, creator)`;宏用 ##拼接,必须放 namespace 内裸类名调用;app 骨架=继承 systems::phone::App,构造传 icon,实现 run/back/init/cleanResource)。出厂 SPIFFS assets 只有 xiaozhi 表情 PNG/srmodels,无 brookesia 图标。
- **出厂固件构成(官方文档证实)**:factory 分区 = Demo/esp-brookesia 单 app(只有 Squareline,“Settings/多 app”是 xiaozhi 固件的 UI);示例程序与小智固件是两个独立程序,无法共存/切换。
