...
esp_err_t bsp_display_new(const bsp_display_config_t *config, esp_lcd_panel_handle_t *ret_panel, esp_lcd_panel_io_handle_t *ret_io)
{
    ...
    /* DMA directly from PSRAM LVGL buffers: without this flag every color
     * flush bounce-allocates the full transfer size from internal DMA heap
     * (46 KB per 466x50 band) and fails with ESP_ERR_NO_MEM once heap
     * fragments. LVGL band buffers are 4-byte aligned (466*2B rows), which
     * satisfies EDMA alignment. */
...
static lv_display_t *bsp_display_lcd_init(const bsp_display_cfg_t *cfg)
{
    ...
    esp_lv_adapter_display_config_t disp_cfg = {
        ...
        /* TE sync patch (not in upstream BSP 3.0.0): CO5300 TE output is on
         * BSP_LCD_TE (GPIO13), TEARON (0x35 0x00) already sent in
         * lcd_init_cmds above. Timing fields left 0 -> adapter uses 60 Hz
         * defaults; only consumed when app selects TEAR_AVOID_MODE_TE_SYNC. */
        .te_sync = {
            .gpio_num = BSP_LCD_TE,
            .bus_freq_hz = 40 * 1000 * 1000,
            .data_lines = 4,
            .bits_per_pixel = 16,
        },
    };
